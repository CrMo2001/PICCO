import{c as o,B as t,o as d}from"./index-Baoj87yD.js";const a={class:"markdown-body"},s={__name:"04duplicate",setup(l,{expose:c}){return c({frontmatter:{}}),(p,e)=>(d(),o("div",a,e[0]||(e[0]=[t(`<h1>学习使用 duplicate</h1><p>在本节教程中，我们将创建一个基于现金堆叠的图表。这个图表的特点是每个柱子由多个现金图片堆叠而成，堆叠的数量由数据决定，并且每个柱子顶部显示对应的数值标签。我们将逐步解析代码，并解释每一步的作用。</p><h2>1. 准备数据</h2><p>首先，我们定义了一组数据，表示不同类别的数值：</p><pre><code class="language-typescript">const data = [
...
</code></pre><ul><li><code>label</code>：文本对象，字体大小为 <code>30</code>，居中对齐，字体加粗。</li><li><code>encode</code>：将文本内容与数据的 <code>amount</code> 字段绑定。</li></ul><h2>5. 组合现金堆叠和标签</h2><p>我们将现金堆叠 <code>cashStack</code> 和标签 <code>label</code> 组合在一起，并设置它们之间的约束关系。</p><pre><code class="language-typescript">const glyph = combine([cashStack, label]).connect(label, cashStack, { anchor: [0.5, 1] }) // 将标签连接到现金堆叠的顶部中心
</code></pre><ul><li><code>combine</code>：将现金堆叠和标签合并为一个整体。</li><li><code>connect</code>：将标签连接到现金堆叠的顶部中心位置。</li></ul><h2>6. 重复图形</h2><p>使用 <code>repeat</code> 操作符，将组合后的图形 <code>glyph</code> 重复多次，每个图形对应一个数据点。</p><pre><code class="language-typescript">const collection = repeat(glyph)
...
</code></pre><ul><li><code>xAxis</code>：x 轴，表示类别，不显示刻度线。</li></ul><h2>8. 组合图形并渲染</h2><p>最后，我们将图形和 x 轴组合在一起，并使用 <code>show</code> 函数将其渲染到页面上。</p><pre><code class="language-typescript">const chart = combine([collection, xAxis])

show([[chart, data]], document.body, {})
</code></pre><ul><li><code>combine</code>：将图形和 x 轴合并为一个整体。</li><li><code>show</code>：将图形渲染到 <code>document.body</code> 中。</li></ul><h2>9. 最终效果</h2><p>运行以上代码后，你将看到一个基于现金堆叠的图表：</p><ul><li>每个柱子由多个现金图片堆叠而成，堆叠的数量由数据的 <code>amount</code> 字段决定。</li><li>每个柱子顶部显示对应的数值标签。</li><li>x 轴显示类别。</li></ul>`,37)])))}};export{s as default};
